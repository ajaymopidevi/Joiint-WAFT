...
def normalize_coords(grid):
    ...
    h, w = grid.size()[2:]
    grid[:, 0, :, :] = 2 * (grid[:, 0, :, :].clone() / max(w - 1, 1)) - 1  # x: [-1, 1]
    grid[:, 1, :, :] = 2 * (grid[:, 1, :, :].clone() / max(h - 1, 1)) - 1  # y: [-1, 1]
...
def disp_warp(feature, disp, padding_mode='border'):
    ...
    warped_feature = F.grid_sample(feature, sample_grid, mode='bilinear', padding_mode=padding_mode, align_corners=True)
    return warped_feature

def flow_warp(feature, flow, padding_mode='border'):
    grid = meshgrid(feature)  # [B, 2, H, W] in image scale
